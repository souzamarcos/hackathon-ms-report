...
public class TokenJwtUtils {
    ...
    public Employee getEmployeeFromToken(String token) {
        var id = getEmployeeIdFromToken(token);
        var email = getEmployeeEmailFromToken(token);
        var name = getEmployeeNameFromToken(token);
        var type = EmployeeType.values()[getEmployeeTypeFromToken(token)];
        return new Employee(id, email, name, type);
    }
    ...
    public String getEmployeeEmailFromToken(String token) {
        return Optional.ofNullable(readToken(token)
            .getClaim("email")
            .asString()).orElseThrow(() -> new JWTVerificationException("Token claim 'email' required"));
    }
    ...
    public String getEmployeeNameFromToken(String token) {
        return Optional.ofNullable(readToken(token)
            .getClaim("name")
            .asString()).orElseThrow(() -> new JWTVerificationException("Token claim 'name' required"));
    }
    ...
    public Integer getEmployeeTypeFromToken(String token) {
        return Optional.ofNullable(readToken(token)
            .getClaim("type")
            .asInt()).orElseThrow(() -> new JWTVerificationException("Token claim 'type' required"));
    }
